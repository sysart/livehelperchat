...
class erLhcoreClassTransfer
{
    ...
    public static function getTransferChats($params = array())
    {
       ...
       if (isset($params['department_transfers']) && $params['department_transfers'] == true) {
	        ...
	        if (isset($params['only_active']) && $params['only_active'] == true) {
	          $limitationSQL .= ' AND lh_chat.status!=' . erLhcoreClassModelChat::STATUS_CLOSED_CHAT;
	        }
	        ...
	        $stmt = $db->prepare('SELECT lh_chat.*,lh_transfer.id as transfer_id FROM lh_chat INNER JOIN lh_transfer ON lh_transfer.chat_id = lh_chat.id WHERE lh_transfer.transfer_to_user_id = :user_id '.$limitationSQL.' ORDER BY lh_transfer.id DESC');
